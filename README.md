...
| [`loop`](plugins/software-engineering-skills/skills/loop/README.md) | Routes one document to the loop that can run it — sliced design doc or story map to `delivery-loop`, approved plan to `implementation-loop` — classifying from content, never the filename |
| [`delivery-loop`](plugins/software-engineering-skills/skills/delivery-loop/README.md) | Turns a sliced design doc or story map into a queue that writes each slice's spec and plan, then builds every task that plan contains — the board grows itself as the plans are written |
