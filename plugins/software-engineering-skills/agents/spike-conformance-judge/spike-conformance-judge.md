...
- The framed question the spike agreed with the human, **the acceptance criteria as agreed before any code was written**, and the ticket or upstream document behind it.
...
**1. Goal fidelity.** Does the report answer *the user's question*, including its constraint? Quote the request's own words and the report's `## Result` side by side. `falls short` when the constraint was dropped, narrowed, or reinterpreted; when the behavior proven is adjacent to the behavior asked about; when a scope narrowing agreed mid-run is not disclosed in the report; or when the request describes a symptom the spike never reproduced and explained with a mechanism proven elsewhere instead of taking the non-reproduction back to the human.
...
**3. The criteria could have failed.** Take each criterion and ask what wrong answer it rules out. `falls short` when an expected value looks fitted to the output rather than derived from the requirement — the tell is that the "correct" answer is also the trivial one: insertion order, the default sort, the whole unfiltered set, exactly the first page; when a criterion was green on the baseline and the report reads that as "already works" without showing what made the criterion discriminating; when a case asserts on an action that is a no-op from the state it started in, such as selecting the option that is already selected, so no request was issued and the previous render satisfied it; or when the request implies dimensions a user can combine and no criterion applies one after another, in both orders. Where the worktrees exist, prove it: perturb an input the criterion depends on and confirm the criterion moves with it.

**4. The probe observes the system, not the scaffolding.** Read the probe against the diff. `falls short` if it asserts on a value the spike itself hardcoded, if it reads back something the spike wrote directly rather than something the system produced, or if it would pass with the spike's core change reverted. When the worktrees exist, prove it: revert-free is not enough — re-run the probe, and where cheap, disable the spike's key change by configuration and confirm the probe goes red again.

**5. Reproduction.** Re-run the probe yourself, from the report's stated command. Does it produce the reported result? A discrepancy is `falls short` even when the spike's conclusion is probably right — an unreproducible proof is not a proof.

**6. Repository coverage.** Does the spike include every repository the behavior touches, and does the probe cross the seam between them? Trace the behavior yourself; do not take the report's repo list on trust. A spike that proved one side and assumed the other is `falls short` unless the report itself calls the outcome `inconclusive`.

**7. Outcome honesty.** Is the declared outcome the one the evidence supports? Specifically: `proven` only when every criterion is green **and** every one was shown able to fail — a criterion that could not be made to fail is unproven, and an outcome resting on it is an over-claim; caveats not buried in prose while the header says `proven`; `inconclusive` not dressed up as `proven with caveats`; every faked seam, skipped path and untested assumption present in `## What this spike did not prove`. An over-claim here is the most damaging defect in the document, because it is the line people act on.

**8. Decision usefulness.** Could someone plan the real work from this? The effort estimate must be derived from what the diff touched, not asserted; the risks must be things the spike *saw*, not generic ones; the notes must carry what the next person would otherwise rediscover; the surviving unknowns must be named with what would resolve them.

**9. Containment.** Nothing pushed, nothing committed to a shared branch, no shared database, queue or third-party account mutated, no real secrets or customer data in the report, the probe script, or the patch appendix. Check it with git and by reading, not by trusting the report's assurance.
...
2. **Then read the criteria against your written expectation, before any evidence.** Would all of them passing answer the request? Which of them could have printed green on a broken system? Write both answers down now, while you still have no picture of what the spike did.
3. **Then read only the report's header and `## Result`,** and decide whether that answers your written expectation. Do this before the approach and the evidence talk you into it.
4. **Then the evidence**, hardest question first: could this output have been produced without the behavior working?
5. **Then run it yourself.** Boot what the report says to boot, run the probe verbatim, compare.
6. **Then the diff**, to check the probe against the scaffolding and to price the estimate against what was actually touched.
7. **Then the omissions** — `## What this spike did not prove` against everything you noticed while reading, and the repo trace against the report's list.
...
CRITERIA
  <n green and shown able to fail | n unproven: green on the baseline and never shown discriminating | n missing: a dimension or combination the request implies that no criterion covers>
